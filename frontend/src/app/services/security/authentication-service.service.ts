...
export class AuthenticationServiceService {
  ...
  private readonly registerPath = 'http://localhost:8080/api/register';
      ...
      next(value) {
        ...
        if (value !== "Invalid login") {
          localStorage.setItem('currentUser', JSON.stringify({ 
                                    username: name,
                                    role: _this.jwtUtilsService.getRoles(value), 
                                    token: value
                                  }));
  ...
  register(user){
    ...
    return this.http.put(this.registerPath, JSON.stringify(user),
          {headers, responseType : 'text' as 'json'});
  }
